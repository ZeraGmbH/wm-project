...
void cPCBIFace::ActionHandler(int entryAHS)
{
    ...
    case GetStatusOVLFinished:
    case ResetStatusOVLFinished:
    ...
    case GetStatusOVLStart:
	SendGetStatusOVLCommand();
	AHS++;
	break;
	
    case ResetStatusOVLStart:
	SendResetStatusOVLCommand();
	AHS++;
	break;
...
void cPCBIFace::getStatusOVL()
{
    m_ActTimer->start(0,GetStatusOVLStart);       
}


void cPCBIFace::resetStatusOVL()
{
    m_ActTimer->start(0,ResetStatusOVLStart);       
}
...
void cPCBIFace::SendGetStatusOVLCommand()
{
    QString cmds = QString("sens:stat:over?\n");
    iFaceSock->SendQuery(cmds);        
}


void cPCBIFace::SendResetStatusOVLCommand()
{
    QString cmds = QString("sens:stat:over:res\n");
    iFaceSock->SendCommand(cmds);    
}
