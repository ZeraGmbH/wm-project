...
complex cOwnError::CmpOECorrVector()
{
    ...
    if ( (m_pViewData->m_bValid = m_ConfData.m_bOECorrection) == true) {
	double betrag = 0.0; // default keine korrektur
	double winkel = 0.0;
	QStringList relEntriesList;
    if ( (m_pViewData->m_bValid = !m_sOEEntriesList.empty()) == true) { // es gibt gültige einträge in der verwendeten datei
        ...
        if ( (m_pViewData->m_bValid = (relEntriesList.count() > 0)) == true) { // gab es einträge für das gewählte ü-verhältnis ?
...
bool cOwnError::isValidEntry(QString s) 
{
    ...
    if ( !(eP.isCurrent() || eP.withoutUnit()) ) return false; // der 2. auch
    words.pop_front();
