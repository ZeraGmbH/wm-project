...
complex cOwnError::CmpOECorrVector()
{
    ...
    if ((m_pViewData->m_bValid = m_ConfData.m_bOECorrection)) {
        double betrag = 0.0; // default keine korrektur
        double winkel = 0.0;
        QStringList relEntriesList;
        m_pViewData->m_bValid = !m_sOEEntriesList.empty();
        if (m_pViewData->m_bValid) { // es gibt gültige einträge in der verwendeten datei
            ...
            m_pViewData->m_bValid = relEntriesList.count() > 0;
            if (m_pViewData->m_bValid) { // gab es einträge für das gewählte ü-verhältnis ?
...
bool cOwnError::isValidEntry(QString s) 
{
    ...
    eP = words.first();
    if ( !(eP.isVoltage() || eP.withoutUnit()) )
        return false; // der 1. eintrag muss eine spannung sein
    words.pop_front();
    ...
    eP = words.first();
    if ( !(eP.isLoadPoint() || eP.withoutUnit()) )
        return false; // %
    words.pop_front();

    eP = words.first();
    if ( !(eP.isError() || eP.withoutUnit()))
        return false; // %, ppm, ....
    words.pop_front();

    eP = words.first();
    if ( !(eP.isAngle() || eP.withoutUnit()) )
        return false; // °,' ....
    words.pop_front();
